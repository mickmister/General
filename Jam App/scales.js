...
var majorScaleOffsets = [0, 2, 4, 5, 7, 9];

var minorScaleOffsets = [0, 3, 5, 7, 8, 10];


var majorPattern = [createMajorInfo, createMinorInfo, createMinorInfo, createMajorInfo, createMajorInfo, createMinorInfo];
var minorPattern = [createMinorInfo, createMajorInfo, createMinorInfo, createMinorInfo, createMajorInfo, createMajorInfo];


this.getScaleInfo = function(number, callback)
{
	var offsets;
	var pattern;
	if(number % 2 === 0) //major
	...
	for(var i in offsets)
	{
		...
		var index = (number + value) % 12;
		console.log(index);
		var func = pattern[i];
		var chord = func(index);
		toReturn.push(chord);
	}
...
function createMajorInfo(i)
{
	...
	info.rootindex = i % 12;
	info.thirdindex = (i + 4) % 12;
	info.fifthindex = (i + 7) % 12;
